...
const WakatimeDailyWidget: React.FC = () => {
  ...
  if (error) {
    return (
      <div className="sg-bold text-[var(--color-text)] text-xl">{error}</div>
    );
  }

  return (
    <div className="flex flex-col gap-2">
      <p className="sg-bold text-[var(--color-text)] text-xl">Today's Coding Time</p>
      <div className="flex items-center gap-3 rounded-xl px-3 py-2 w-full max-w-md hover:scale-[1.03] transition-all duration-200 ease-in-out">
        ...
        <span className="text-sm md:text-base sg-medium text-[var(--color-text)] truncate">
          {timeText}
        </span>
      </div>
    </div>
